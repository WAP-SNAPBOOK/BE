...
- `tags[].id`는 `shop_tags.id` 기준이다.
- 메뉴 수정 모달의 기존 카테고리 기본값은 `menu.tags`를 사용한다.
- 단일 카테고리 UI에서는 `menu.tags[0]?.id`를 기본 선택값으로 사용할 수 있다.
- 다중 카테고리 UI로 확장하면 `menu.tags.map(tag => tag.id)`를 선택 목록으로 사용한다.
...
- 메뉴 조회 응답의 `tags[].id`도 같은 `shop_tags.id` 기준이므로 연결/해제 API에 그대로 사용할 수 있다.
- 메뉴 추가 모달에서 카테고리를 선택하게 한 뒤 메뉴 생성 성공 후 연결 API를 호출한다.
- 메뉴 수정에서 카테고리를 바꾸는 경우:
  - `menu.tags`의 기존 카테고리와 사용자가 선택한 카테고리를 비교
  - 새 카테고리 연결 API 호출
  - 제거된 카테고리 해제 API 호출
...
- 메뉴 카테고리 전체 교체 API는 아직 없다. 수정 화면에서는 현재 `menu.tags`와 선택값을 비교해서 연결/해제 API를 조합해야 한다.
