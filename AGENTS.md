...
- 구현 시에는 모든 구현을 절대 한꺼번에 하지 않는다.
- "지금 필요한 가장 작은 다음 변경" 에 대한 구현을 한 뒤, worklog를 작성한 뒤 커밋하는 것이 한 사이클이다.
- 구현 계획은 phase 목록이 아니라 최소 변경 단위 목록으로 작성한다.
- 각 변경 단위는 `목표`, `분류`, `수정 대상`, `검증`, `완료 조건`을 포함한다.
- `분류`는 `Structural` 또는 `Behavioral` 중 하나를 사용한다.
- `Structural`과 `Behavioral`은 구현 단계 이름이 아니라 각 변경 단위의 속성으로 사용한다.
- 하나의 변경 단위는 하나의 분류만 가진다.
- "지금 필요한 가장 작은 다음 변경"에 대한 구현은 구조적 변경과 동작 변경이 있다.
    - **구조적 변경(Structural)**: 이름 변경/추출/이동 등, 동작은 바꾸지 않는 정리
    - **동작 변경(Behavioral)**: 기능 추가/수정/버그 수정 등 동작을 바꾸는 변경
- 구조 변경과 동작 변경을 한번에 하지 않는다.
- 구조 변경과 동작 변경 중에는 구조 변경을 먼저한다.
...
- 워크트리에서 작업하고 있다면 문서 기준 위치는 워크트리 작업 디렉터리 내의 `docs/issues/#<issue-number>/...` 이다.
- 구현 전 `docs/issues/#<issue-number>/02-analysis/0001-implementation-plan.md` 를 작성하거나 갱신한다.
- 구현은 "지금 필요한 가장 작은 다음 변경" 단위로 진행한다.
...
- `clarify-feature-specs`: 모호한 요구사항을 질문 라운드로 구체화해 `PRD`, `Feature Spec`, `Tech/API Spec`, `Acceptance Criteria` 4개 문서로
  정리한다. 기본 경로는 `docs/specs/<feature-name>/`이다.
